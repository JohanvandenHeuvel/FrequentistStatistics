...
Lecturer = read.table(file.choose(), header=TRUE, sep="\t", quote="", comment.char="", fill=TRUE)
error_bar = ggplot(Lecturer, aes(job,friends))
error_bar + stat_summary(fun.y = mean, geom = "bar", position="dodge") + stat_summary(fun.data = mean_cl_normal, geom = "errorbar", position=position_dodge(width=0.90)) + labs(x = "Job", y = "Mean of Friends")

#2 Using this same lecturer data, plot an error bar chart showing the *median* alcohol consumption for students and lecturers. This time, make the bars themselves green. Don't forget axis labels. Use a different geom for plotting the confidence intervals than the one you used in question 1. How do the medians compare to the means for alcohol consumption?
error_bar_alcohol = ggplot(Lecturer, aes(job, alcohol))
error_bar_alcohol + stat_summary(fun.y = median, geom = "bar", position="dodge", fill = "green") + stat_summary(fun.data = mean_cl_normal, geom = "errorbar", position=position_dodge(width=0.90)) + labs(x = "Job", y = "Median alcohol consumption") + stat_summary(fun.data = mean_cl_normal, geom = "pointrange")


#3a Now create a scatterplot with regression lines of alcohol consumption and neuroticism grouped by lecturer/student. 
scatter = ggplot(Lecturer, aes(alcohol, neurotic, colour = job))
scatter + geom_point() + geom_smooth(method = "lm") + labs(title = "Alcohol consumption and neuroticism Scatterplot", x="Alcohol consumption", y="Neurotiscism", colour = "job") 
#3b Now see if you can also plot a regression line that summarizes ALL the data (not grouped by job) in addition to the job-specific regression lines. Remember that if you set colour to vary by job when you define the graph object, this will filter down to all geoms (including the geom_smooth that you use for the general summary regression line!). To avoid this, you can set colour to vary by job only at certain individual levels (i.e., not at the geom_smooth for the general regression line).
graph = ggplot(Lecturer, aes(alcohol,neurotic))
graph + geom_point() geom_smooth(method = "lm") + geom_smooth(method = "lm") + mapping = aes(colour = job) + labs(title = "Alcohol consumption and neuroticism Scatterplot", x="Alcohol consumption", y="Neurotiscism", colour = "job")
...
#4 Load Gifts.csv. Plot a clustered error bar chart of the mean number of gifts given to oneself versus to one's partner for male and female. Include confidence intervals. See if you can figure out how to add a scale_fill_manual() layer that customizes the color of the bars.

...
ViagraData <- read.table("Viagra.dat",sep="\t",header=TRUE)
head(ViagraData)
...
#Normality can be a problem if sample sizes are quite different. 
#In such a case, there is an approach known as the Kruskall-Wallis test that you can use. 
#If sample sizes are equal, don't worry about normality.

#Check Homogeneity of variance
leveneTest(ViagraData$libido, ViagraData$dose, center = "median")


###When doing a simple one-way ANOVA, we use the aov() function instead of lm() because 
#(1) we don't necessarily want the dummy contrasts, and 
#(2) we need sum of squares info to calculate an effect size
...
#First, let's try dummy contrasts. This is how we assign contrast variables to a nominal variable. 
#We use contr.treatment function (treatment contrasts are any kind of contrast where all other means are compared to some level; 
#default level is reference level, which is the same as dummy contrasts).
...
###If we didn't have hypotheses about which means should be different from one another, 
#we could run post-hoc testing, which looks at every possible pairwise comparison between means. 
#Let's use the Benjamini-Hochberg correction ("BH") instead of the more conservative and conventional Bonferroni correction.
...
#Reporting: All significant values are reported at p < .05. 
#There was a significant effect of dose on libido, F(2, 12) = 5.119, omega^2 = .354. 
#Planned contrasts revealed that high dose and low dose lead to higher libido than placebo, t(12) = 2.474, r = .58, 
#and that high dose lead to higher libido than low dose, t(12) = 2.029, r = .51.
